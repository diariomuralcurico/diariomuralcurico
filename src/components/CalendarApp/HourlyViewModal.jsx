...
function HourlyViewModal({ show, onClose, date, events, setEvents, setShowDialog, setNewEvent, handleEditEvent, showDialog }) {
  useEscapeKey(show && !showDialog, onClose); // Hook para cerrar el modal al presionar Escape sólo cuando el modal está abierto y no se está mostrando el diálogo de evento
  ...
  const dayEvents = events
    .filter((event) => {
      ...
      const eventStartDate = new Date(event.date).setHours(0, 0, 0, 0);
      const eventEndDate = new Date(event.fechaFin).setHours(23, 59, 59, 999);
    ...
    .map((event) => {
      ...
      // For recurring events, find the specific recurrence date and use its time
      if (event.recurrence !== 'None') {
        ...
        if (matchingRecurrence) {
          ...
          const [hours, minutes] = event.endTime ? event.endTime.split(':').map(Number) : [23, 59];
  ...
  const timeToMinutes = (timeStr) => {
    if (!timeStr || typeof timeStr !== 'string' || !/^\d{2}:\d{2}$/.test(timeStr)) return 0;
  ...
  return (
    <div className={`fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-60 ${show ? '' : 'hidden'}`}>
      <div className="bg-white p-4 md:p-6 rounded-lg shadow-lg w-full max-w-3xl max-h-[85vh] flex flex-col">
        <div className="flex-shrink-0 mb-4">
          <h2 className="text-xl font-semibold text-center">
            Eventos del {date ? date.toLocaleDateString('es-CL', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }) : ''}
          </h2>
        </div>
          ...
          <div className="relative" style={{ height: `${totalHeight}px` }}>
            {hours.map((hour, index) => (
              <div
                ...
                style={{ top: `${index * hourRowHeight}px`, height: `${hourRowHeight}px` }}
              >
                <div
                  className="absolute text-xs text-gray-500 text-right pr-2"
                ...
                <button
                  ...
                  className="absolute right-1 top-1 w-5 h-5 flex items-center justify-center bg-indigo-100 text-indigo-600 rounded-full hover:bg-indigo-200 transition duration-200 text-xs z-30"
            ...
            {eventsWithLanes.map((event) => {
              ...
              const laneFraction = effectiveMaxLane > 1 ? event.lane / effectiveMaxLane : 0;
              const widthFraction = effectiveMaxLane > 1 ? 1 / effectiveMaxLane : 1;
              ...
              return (
                <div
                  key={event.id}
                  className="absolute rounded px-2 py-1 text-white text-xs overflow-hidden shadow border border-black border-opacity-20 z-20 cursor-pointer hover:opacity-90"
                  style={{
                    top: `${topPosition}px`,
                    height: `${Math.max(eventHeight, 20)}px`,
                    left: eventLeft,
                    width: eventWidth,

...
describe('Reservas Component', () => {
  ...
  it('handles event submission with valid data', async () => {
    ...
    await user.type(screen.getByLabelText('Nombre del Evento'), 'Fiesta de la Vendimia');
    await user.type(screen.getByLabelText('Descripción'), 'Celebración del vino en Curicó');
    await user.type(screen.getByLabelText('Fecha y Hora de Inicio'), '2025-04-30T10:00');
    await user.type(screen.getByLabelText('Fecha y Hora de Fin'), '2025-04-30T18:00');
    await user.type(screen.getByLabelText('Dirección'), 'Plaza de Armas, Curicó');
    await user.selectOptions(screen.getByLabelText('Tipo de Precio'), 'Gratuito');
    ...
    await waitFor(() => {
      expect(collection).toHaveBeenCalledWith(db, 'menu_test');
      expect(addDoc).toHaveBeenCalledWith('menu_test', expect.objectContaining({
  ...
  it('handles errors during event submission', async () => {
    ...
    await waitFor(() => {
      expect(consoleSpy).toHaveBeenCalledWith('Error al guardar el evento:', expect.any(Error));
    });
